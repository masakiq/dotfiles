...
local function grep_string_additional_args(opts)
  return function()
    return sort_args(opts)
  end
end

local function live_grep_additional_args(opts)
  return function()
    local args = sort_args(opts)

    if opts.fixed_strings ~= false then
      table.insert(args, 1, "--fixed-strings")
    end
...
function M.grep_string(opts)
  ...
  picker_opts.additional_args = grep_string_additional_args(opts)
...
function M.live_grep(opts)
  ...
  local picker_opts = core.with_file_mappings(core.default_picker_opts(opts))
  picker_opts.search_dirs = normalize_search_dirs(opts.search_dirs)
  picker_opts.additional_args = live_grep_additional_args(opts)
